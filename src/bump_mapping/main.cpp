...
{
public:
	void apply(osg::Node &node){ osg::NodeVisitor::traverse(node); };
	...
	void apply(osg::Geode &node)
	{
		for (unsigned int i = 0; i< node.getNumDrawables(); ++i)
		{
			...
			if (geom) this->generateTangentArray(geom);
		}
		traverse(node);
	}
...
int main(int argc, char **argv)
{
	...
	osg::ref_ptr<osg::Node> scene = osgDB::readNodeFile("skydome.osgt");
	...
	prog->addBindAttribLocation("binormal", 7);
	...
	state->addUniform( new osg::Uniform("colorTex", 0));
	state->addUniform( new osg::Uniform("normalTex", 1) );
	...
	state->setTextureAttributeAndModes(0, colorTex.get(), value);
	state->setTextureAttributeAndModes(1, normalTex.get(), value);


	osgViewer::Viewer viewer;
	//viewer.setCamera(rttCamera);
